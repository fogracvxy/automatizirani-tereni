...
danas = datetime.today().strftime('%Y-%m-%dT%H:%M:%S')
    ...
    if (item["due"] and item['description']):
        ...
        if danasdatum  < datumtaska and danasdatum + timedelta(1)  > datumtaska:
            with open('Batchgeoinfo.txt', 'a', encoding='utf-8') as f:
                ...
                print(item["content"]+"\t" + str(datumtaska) +"\t"+listaustring, file=f)
...
datoteka = os.path.abspath("Batchgeoinfo.txt")
start_firefox("batchgeo.com")
...
click("Map Your Data")
#naslov karte se sastoji od danasnjeg datuma + tereni npr. "2021-09-01T07:46:22  Tereni"
write(danas + ' Tereni', into="Title")
write('marin@geo-kom.hr', into="Email")
